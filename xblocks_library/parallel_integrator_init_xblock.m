...
% overflow: Accumulator overflow behavior. For CIC filter default to "Wrap"
...
for i=1:n_inputs,
    xlsub3_addertree{i} = xBlock(struct('source', str2func('adder_tree_init_xblock'),'name', ['adder_tree', num2str(i)]), ...
                                 {[blk,'/','adder_tree', num2str(i)], 'n_inputs', n_inputs, 'add_latency', add_latency, 'quantization', 'Round  (unbiased: +/- Inf)', 'overflow', 'Wrap', 'mode', 'Behavioral'}, ...
...
sync_out.bind(sync);  % just leave it here for now


if ~isempty(blk)
    if ~strcmp(blk(1), '/')
        clean_blocks(blk);
        fmtstr = sprintf('Add Latency = %d\nBitwidth = %d bin_pt = %d\n%s', add_latency, n_bits, bin_pt,overflow);
