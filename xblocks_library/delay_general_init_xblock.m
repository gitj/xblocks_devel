...
switch delaytype
	case 'delay_bram'
		...
		delay_bram_sub = xBlock(delay_bram_config,{[blk,'/',delay_bram_config.name], DelayLen,bram_latency,use_dsp48});
	...
	case 'delay_slr'
		...
		delay_slr_sub = xBlock(delay_slr_config,{[blk,'/',delay_slr_config.name], DelayLen});
	...
	case 'sync_delay'
		...
		sync_delay_sub = xBlock(sync_delay_config,{[blk,'/',sync_delay_config.name], DelayLen});
	...
	case 'sync_delay_en'
		...
		sync_delay_en_sub = xBlock(sync_delay_en_config,{[blk,'/',sync_delay_en_config.name], DelayLen});
	...
	case 'sync_delay_prog'
		...
		sync_delay_prog_sub = xBlock(sync_delay_prog_config,{[blk,'/',sync_delay_prog_config.name], MaxDelay});
	...
	case 'window_delay'
		...
		window_delay_sub = xBlock(window_delay_config,{[blk,'/',window_delay_config.name], DelayLen});
	...
	case 'pipeline'
		...
		pipeline_sub = xBlock(pipeline_config,{[blk,'/',pipeline_config.name], DelayLen});
...
if ~isempty(blk) && ~strcmp(blk(1), '/')
    clean_blocks(blk);
    annotation = get_param([blk,'/',delaytype], 'AttributesFormatString');
