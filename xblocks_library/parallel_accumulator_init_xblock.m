...
% delay_len: abandoned parameter:  length for delay elements; 
%               at first it was an attempt to accomondate different clock
%               rate but I don't think it's necessary
...
% abandoned parameter, force it to be 1
if delay_len ~=1
    delay_len = 1;
end
...
for i =1:n_inputs
    ...
    adder_tree_blks{i} = xBlock(struct('source', str2func('adder_tree_init_xblock'),'name', ['adder_tree',num2str(i)]), ...
                     {[blk,'/','adder_tree',num2str(i)],'n_inputs', len, 'add_latency', add_latency, 'quantization', 'Round  (unbiased: +/- Inf)', 'overflow', 'Saturate', 'mode', 'Behavioral'}, ...
